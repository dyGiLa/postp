...
measureData_h30mT_tauQ150 = pd.read_csv('/home/heidi/Documents/dyGiLa-project/dyGiLa-data/dyGiLa-Langevin/thermal-bath-UniT-Hfield-quenches/lumi-runs/thermal-bath-UniT-Hfield-quenches/LUMI-C/tauQ125-tauQ3000-H30-RSeed-1-periodicBC/p-26.0-tauQ50-3050-Tdpendentgamma/p-26.0-tauQ150/stats/measure-stream.csv', sep=' ', header=None)
...
#print(freqNP)
#print(abs(sp))
ax1.loglog(freq, np.abs(sp), linewidth=LineWidth, label=r'$<\Delta>(\nu)-0,\tau_{Q}=1550t_{GL}}$', linestyle='solid', color=(1.0, 0.5, 0.0))

###################
### tauQ = 1550 ###

# set up time points between which Delta doesn't reach bulk equlibrium
startTime = 500
bulk_eqTime = 1150 # can be 1400
t      = measureData_h30mT_tauQ1550.values[:,0]
Deltat = measureData_h30mT_tauQ1550.values[:,4]
...
index_t_after_pick = np.where(t >= startTime)[0][0]
index_t_before_pick = np.where(t <= bulk_eqTime)[0][-1]
#print(np.where(t >= startTime))
#print(np.where(t <= bulk_eqTime))
print(index_t_after_pick)
print(index_t_before_pick)

t_pick = t[index_t_after_pick:index_t_before_pick] - startTime
print(t_pick)
Deltat_pick = Deltat[index_t_after_pick:index_t_before_pick]
print(Deltat_pick)
...
freqNP = fftshift(fftfreq(t_pick.shape[-1],2.0))
#freqNP = fftfreq(t_pick.shape[-1],2.0)
#print(freqNP.shape)
...
#print(freqNP)
#print(abs(sp))
ax1.loglog(freq, np.abs(sp), linewidth=LineWidth, label=r'$<\Delta>(\nu)-II,\tau_{Q}=1550t_{GL}}$', linestyle='solid', color=(1.0, 0, 1.0))
...
#print(freqNP)
#print(abs(sp))
ax1.loglog(freq, np.abs(sp), linewidth=LineWidth, label=r'$<\Delta>(\nu)-III,\tau_{Q}=1550t_{GL}}$', linestyle='solid', color=(0, 1.0, 0.0))
...
# freqNP = fftshift(fftfreq(t_pick.shape[-1],2.0),2.0)
# #print(freqNP.shape)
