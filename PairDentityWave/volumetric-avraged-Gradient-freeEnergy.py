...
for i, folder in enumerate(seed_folders):
    ...
    try:
        t, glGradfe = load_GLGradFE(folder)
...
ax.set_ylabel(
    r' Grad Energy Density '
    r'$f^{V}_{Grad}/\frac{1}{3}k_{B}T_{C}.{(\xi^0_{GL})}^{-3}$',
    fontsize=20
)

ax.set_xlim(0, 5000)    
ax.set_ylim(84.5, 85.5)

ax.set_title(
    fr'Volumetric Averaged Gradient Energy density with $1.1{{\mu}}m$ $T=0.689mK$ at $0bar$',
    fontsize=20,
    pad=25
)
...
save_path = os.path.join(base_folder, "GLGrad_D-1.1um-p-0.0bar-T-0.68mK.png")
