...
def load_GLBulkFE(folder_path):
    """
    Load measure-stream.csv and compute GLBulkFE using NumPy arrays.
...
for i, folder in enumerate(seed_folders):
    ...
    try:
        t, glGradfe = load_GLBulkFE(folder)
...
ax.set_ylabel(
    ...
    r'$\{f^{V}_{B}\}/\frac{1}{3}k_{B}T_{C}.{(\xi^0_{GL})}^{-3}$',
    fontsize=20
)

ax.set_xlim(0, 5000)    
ax.set_ylim(-0.875, -0.86)

ax.set_title(
    fr'Volumetric Averaged Bulk Energy density with $1.1{{\mu}}m$ $T=0.689mK$ at $0bar$',
    fontsize=20,
    pad=25
)
...
save_path = os.path.join(base_folder, "GLBulkFE_D-1.1um-p-0.0bar-T-0.689mK.png")
