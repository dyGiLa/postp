...
base_dir = '/home/heidi/ReHD/dyGiLa-project/dyGiLa-data/dyGiLa-Langevin/thermal-bath-blob-quenches-Hfield/lum-runs/lumi-G/project_462000960/heterogenouos-quench/cube-1024/H-00mT/p-23bar/'
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
    ...
    for part in parts:
        ...
        match = re.match(r".*Ttdb1-([0-9.]+)", part)
        if match:
            ptauQ = match.group(1)
            break
    else:
...
# # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
    ...
    ax2.set_ylim(-0.000002, 0.000001)
