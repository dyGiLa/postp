...
'''
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
# >>>>>>>>>    bdefine low path filter      <<<<<<<<<<<<<< #
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #

def lowpass_filter(data, cutoff_freq_inv_tGL, dt, order=4):

    fs = 1.0 / dt                    # "sampling frequency", in 1/tGL
    nyq = 0.5 * fs                   # Nyquist frequency, in 1/tGL
    normal_cutoff = cutoff_freq_inv_tGL / nyq  # Normalize cutoff
...
base_dir = '/home/heidi/ReHD/dyGiLa-project/dyGiLa-data/dyGiLa-Langevin/thermal-bath-blob-quenches-Hfield/lum-runs/lumi-G/project_462000960/heterogenouos-quench/cube-2048/H-00mT/p-22bar/RSeed-1728-T1-1.5-7.5-t1-0.005'
...
calibrate_to_Tc = True

# Ignore any signal that fluctuates faster than once every N tGLtGL​ units
cutoff_freq_inv_tGL = 0.005
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
    ...
    for part in parts:
        ...
        match = re.match(r"p-22\.0-T1-([0-9.]+)", part)
        if match:
            Ttdb1 = match.group(1)
            break
    else:
...
# # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
    ...
    d2Vrp5d2t = np.gradient(dVrp5dt, dt)
    d2Vrp9d2t = np.gradient(dVrp9dt, dt)
    d2Vrp3d2t = np.gradient(dVrp3dt, dt)
    d2Vrp1d2t = np.gradient(dVrp1dt, dt)

    d2Vrp5d2t_filted=lowpass_filter(d2Vrp5d2t, cutoff_freq_inv_tGL, dt, order=4)
    ...
    ax2.plot(df["t"], d2Vrp5d2t, linewidth=LineWidth, label=fr'$\frac{{d^{{2}}p_B}}{{d^{{2}}t}}\,, T_1={Ttdb1}T_C$', linestyle=line_styles[0], color=lineColors[0])

    ax2.plot(df["t"], d2Vrp5d2t_filted, linewidth=LineWidth, label=fr'$\frac{{d^{{2}}p_B}}{{d^{{2}}t}} filted\,, T_1={Ttdb1}T_C$', linestyle=line_styles[0], color=lineColors[1])
    
    # ax2.plot(df["t"], d2Vrp9d2t, linewidth=LineWidth, label=fr'$\frac{{d^{{2}}p_A}}{{d^{{2}}t}}\,, T_1={Ttdb1}T_C$', linestyle=line_styles[0], color=lineColors[1])

    # ax2.plot(df["t"], d2Vrp3d2t, linewidth=LineWidth, label=fr'$\frac{{d^{{2}}p_{{plnanr}}}}{{d^{{2}}t}}\,,\tau_Q={ptauQ}t_{{GL}}$', linestyle=line_styles[1], color=lineColors[2])

    # ax2.plot(df["t"], d2Vrp1d2t, linewidth=LineWidth, label=fr'$\frac{{d^{{2}}p_{{bg}}}}{{d^{{2}}t}}\,,\tau_Q={ptauQ}t_{{GL}}$', linestyle=line_styles[0], color=lineColors[6])    
    ...
    ax2.set_ylabel(r'State Acceleration $\frac{d^{2}p(t)}{d^{2}t}$',fontsize = 26.0)

    # calibrate the x-axis
    if calibrate_to_Tc == True:
        # ax2.set_xlim(tTc, df['t'].iloc[-1])
        # ax2.set_xlim(tTc, 16000)
        ax2.set_xlim(0, 2000)
    else:
...
# # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
    ...
    output_path = os.path.join(base_dir, 'pVolume-plot/pVAcceleration-pM1include', plot_name)
