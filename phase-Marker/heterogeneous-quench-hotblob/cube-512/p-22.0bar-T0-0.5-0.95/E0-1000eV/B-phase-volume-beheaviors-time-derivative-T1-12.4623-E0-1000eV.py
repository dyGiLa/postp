...
root_dir = '/home/heidi/ReHD3/dyGiLa-project/project_462000960/heterogenouos-quench/cube-512/H-30mT/p-22bar-T0-0.50-0.95/E0-1000eV'

# Pattern to find all pv.csv files
pattern = os.path.join(root_dir, "RSeed-*-T0-0.50-0.95", "p-22.0-T1-12.4623", "stats", "phaseVolume-stream.csv")
...
# thinwall_Rc = 1306.2283737024222 # nm
...
for csv_file in csv_files:
    ...
    try:
        ...
        if not df.empty:
            ...
            # Extract RSeed using regex
            match = re.search(r'RSeed-(\d+)-', csv_file)
            if match:
                RSeed = int(match.group(1))
            else:
                RSeed = "unknown"

            # --- NEW: compute time derivative ---
            t = df["t"].to_numpy()
            V = df["V_p5_acc"].to_numpy()
            dVdt = np.gradient(V, t)   # handles non-uniform spacing

            # Special linestyle case
            if RSeed_counter == 6:
            ...
            if RSeed_counter == 6:
                ax.plot(t, dVdt, linewidth=LineWidth, label=fr'$Vol_{{B}}\,,RandS\,{RSeed}$', linestyle=line_styles[1], color=lineColors[RSeed_counter])
            else:
                ax.plot(t, dVdt, linewidth=LineWidth, label=fr'$Vol_{{B}}\,,RandS\,{RSeed}$', linestyle=line_styles[0], color=lineColors[RSeed_counter])
...
# ax.axhline(y=197845, label='critic volume', color='red', linestyle='-.', linewidth=4)  

ax.set_xlabel(r'$t/t^{0}_{GL}$',fontsize = 26.0)
ax.set_ylabel(r'B-phase Bubbles Volume Speed/$(\xi^{0}_{GL})^3/t^0_{GL}$',fontsize = 26.0)
ax.set_xlim(100.0, 2500)
...
fig.subplots_adjust(left=0.18, bottom=0.18)  # space for labels
    
plot_name = 'B-phase-Bubble-Beheavior-time-derivative-E0-1000eV-T1-12.4623-AllSamples.png'
