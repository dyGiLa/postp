...
for csv_file in csv_files:
    ...
    try:
        ...
        if not df.empty:
            ...
            if RSeed_counter == 6:
                ax.plot(df["t"], df["V_p5_acc"], linewidth=LineWidth, label=fr'$Vol_{{B}}\,,RandS\,{RSeed}$', linestyle=line_styles[1], color=lineColors[RSeed_counter])
            else:
                ax.plot(df["t"], df["V_p5_acc"], linewidth=LineWidth, label=fr'$Vol_{{B}}\,,RandS\,{RSeed}$', linestyle=line_styles[0], color=lineColors[RSeed_counter])
...
plot_name = 'B-phase-Bubble-Beheavior-E0-1500eV-T1-18.5184-AllSamples.png'
