...
base_dir = '/home/heidi/ReHD/dyGiLa-project/dyGiLa-data/dyGiLa-Langevin/thermal-bath-UniT-Hfield-quenches/lumi-runs/thermal-bath-UniT-Hfield-quenches/LUMI-G/project_462000960/cube-1024/H-00mT/p-32bar/RSeed-1728-tauQ-50-5050tGL'
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
    ...
    freeze_out_t = fo_data[fo_data['tauQ'] == int(ptauQ)]['freeze_out_time'].values[0]
...
# # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
    ...
    ax2.plot(df["t"], d2Vrp9d2t, linewidth=LineWidth, label=fr'$\frac{{d^{{2}}p_A}}{{d^{{2}}t}}\,,\tau_Q={ptauQ}t_{{GL}}$', linestyle=line_styles[0], color=lineColors[0])

    # ax2.plot(df["t"], d2Vrp9d2t, linewidth=LineWidth, label=fr'$\frac{{d^{{2}}p_A}}{{d^{{2}}t}}\,,\tau_Q={ptauQ}t_{{GL}}$', linestyle=line_styles[0], color=lineColors[1])
    ...
    if calibrate_to_Tc == True:
        ...
        ax2.set_xlim(15000, 16000)
        
    ax2.set_ylim(-0.0000001, 0.0000001)
