...
base_dir = '/home/heidi/ReHD/dyGiLa-project/dyGiLa-data/dyGiLa-Langevin/thermal-bath-blob-quenches-Hfield/lum-runs/lumi-G/project_462000960/heterogenouos-quench/cube-1024/H-30mT/p-22.0bar/RSeed-1728-T1-6.84-28.63-t1-0.005'
...
# # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
    ...
    ax2.plot(df["t"], dVrp5dt, linewidth=LineWidth, label=fr'$\frac{{dp_B}}{{dt}}\,,T_1={Ttdb1}T_{{C}}$', linestyle=line_styles[0], color=lineColors[0])

    ax2.plot(df["t"], dVrp9dt, linewidth=LineWidth, label=fr'$\frac{{dp_A}}{{dt}}\,,T_1={Ttdb1}T_{{C}}$', linestyle=line_styles[0], color=lineColors[1])

    ax2.plot(df["t"], dVrp3dt, linewidth=LineWidth, label=fr'$\frac{{dp_{{plnanr}}}}{{dt}}\,,T_1={Ttdb1}T_{{C}}$', linestyle=line_styles[1], color=lineColors[2])

    # ax2.plot(df["t"], dVrp1dt, linewidth=LineWidth, label=fr'$\frac{{dp_{{bg}}}}{{dt}}\,,T_1={Ttdb1}T_{{C}}$', linestyle=line_styles[0], color=lineColors[6])    
